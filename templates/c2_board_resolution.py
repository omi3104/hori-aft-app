...
def generate(fields: dict, uk_fields: dict, output_path,
             meeting_date: str = "", directors: list = None):
    doc = Document()
    ...
    _p(doc, "BOARD RESOLUTION / MINUTES OF THE MEETING", bold=True, size=12, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=10)
    ...
    _p(doc, "Agenda: Appointment of Authorising Officer for UK Business Establishment and Immigration Sponsorship Licence Application.", bold=True, size=11)
    _p(doc)
    ...
    if not directors:
        _p(doc, "[Director names to be inserted]", size=11)
    _p(doc)

    # 2. Chairperson
    _heading(doc, "2. Chairperson")
    chairperson = directors[0] if directors else ao_name
    _p(doc, f"The meeting was chaired by Mr./Ms. {chairperson}.", size=11)
    _p(doc)

    # 3. Purpose
    _heading(doc, "3. Purpose of the Meeting")
    _p(doc,
       f"To consider and approve the appointment of an Authorising Officer for the purposes of "
       f"establishing the company in the United Kingdom and making an application to the Home Office "
       f"for a UK Sponsor Licence under the applicable UK Immigration Rules.", size=11)
    _p(doc)
    ...
    _p(doc, f"Mr./Ms. {chairperson} – Chairperson", size=11)
    _p(doc)
